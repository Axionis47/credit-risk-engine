...
class FeatureEngineer:
    ...
    def compute_all(
        ...
        """
        borrower_ids = borrowers["borrower_id"].values
        ...
        drop_cols = [
            "borrower_id", "is_default", "is_fraud", "lgd_value", "ead_value",
        ]
    ...
    def _compute_aggregation_features(self, transactions: pd.DataFrame) -> pd.DataFrame:
        ...
        monthly_cat = (
            t.groupby(["borrower_id", "month", "merchant_category"])["amount"]
            .sum()
            .reset_index()
        )
        monthly_avg = (
            monthly_cat.groupby(["borrower_id", "merchant_category"])["amount"]
            .mean()
        ...
        # Shannon entropy of spending categories
        cat_totals = t.groupby(["borrower_id", "merchant_category"])["amount"].sum().reset_index()
        borrower_totals = t.groupby("borrower_id")["amount"].sum()

        def compute_entropy(group):
            total = group["amount"].sum()
            if total == 0:
                return 0.0
            probs = group["amount"] / total
            probs = probs[probs > 0]
            return float(-np.sum(probs * np.log2(probs)))

        entropy = cat_totals.groupby("borrower_id").apply(compute_entropy)
    ...
    def _compute_behavioral_features(
        self, borrowers: pd.DataFrame, transactions: pd.DataFrame
    ) -> pd.DataFrame:
        t = transactions.copy()
        t["timestamp"] = pd.to_datetime(t["timestamp"])
    ...
    def _compute_time_series_features(self, transactions: pd.DataFrame) -> pd.DataFrame:
        ...
        def compute_trend(group, col, n_months):
        ...
        trends = monthly.groupby("borrower_id").apply(
            lambda g: pd.Series({
                "spend_trend_3m": compute_trend(g, "total_spend", 3),
                "spend_trend_6m": compute_trend(g, "total_spend", 6),
                "txn_frequency_trend": compute_trend(g, "txn_count", 6),
            })
        )

        # Volatility
        volatility = monthly.groupby("borrower_id")["total_spend"].agg(
    ...
    def _compute_payment_features(self, payments: pd.DataFrame) -> pd.DataFrame:
        ...
        pay_ratio = grp.apply(
            lambda g: (g["amount_paid"] / g["amount_due"].clip(lower=1)).mean()
        )
        pay_ratio.name = "payment_amount_ratio"

        # Consecutive on-time streak (from most recent)
        def consecutive_on_time_streak(group):
            statuses = group.sort_values("due_date")["payment_status"].values
            streak = 0
        ...
        streak = grp.apply(consecutive_on_time_streak)
        streak.name = "consecutive_on_time"

        # Payment trend (trend in days_past_due over last 3 months)
        def payment_trend(group):
            recent = group.sort_values("due_date").tail(3)
            if len(recent) < 2:
                return 0.0
            x = np.arange(len(recent))
            y = recent["days_past_due"].values.astype(float)
            try:
                return float(np.polyfit(x, y, 1)[0])
            except (np.linalg.LinAlgError, ValueError):
                return 0.0

        pmt_trend = grp.apply(payment_trend)
    ...
    def _compute_risk_ratios(self, borrowers: pd.DataFrame) -> pd.DataFrame:
        ...
        df["utilization_x_dti"] = (
            borrowers["credit_utilization_ratio"] * borrowers["debt_to_income_ratio"]
        )
        df["income_stability_proxy"] = (
            borrowers["employment_length_years"] / borrowers["age"].clip(lower=1)
        )
        return df
    ...
    def _encode_categoricals(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        if self.STATE_COLUMN in result.columns:
            if fit and "is_default" in df.columns:
                self._state_target_map = (
                    df.groupby(self.STATE_COLUMN)["is_default"].mean().to_dict()
                )
            if self._state_target_map:
                global_mean = df["is_default"].mean() if "is_default" in df.columns else 0.08
                result["state_encoded"] = result[self.STATE_COLUMN].map(
                    self._state_target_map
                ).fillna(global_mean)
            else:
    ...
    def _handle_missing(self, df: pd.DataFrame, fit: bool = True) -> pd.DataFrame:
        ...
        if "months_since_last_delinquency" in result.columns:
            ...
            result["months_since_last_delinquency"] = result[
